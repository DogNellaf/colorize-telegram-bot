...
def colorize(path):
    ...
    numpy_file = np.load('models/pts.npy')
    Caffe_net = cv.dnn.readNetFromCaffe("models/model.prototxt",
                                        "models/model.caffemodel")
    ...
    cv.imwrite(path, (bgr_output*255).astype(np.uint8))